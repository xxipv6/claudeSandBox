...
4. **使用 checklist 格式**：执行步骤必须使用 Markdown checklist 格式（`- [ ]`）
5. **持续维护 checklist**：每完成一步后将 `- [ ]` 改为 `- [x]`
6. **动态显示范围**：始终显示最多 5 个未完成的步骤
   - 初始状态：显示步骤 1-5
   - 步骤 1 完成后：显示步骤 2-6（步骤 1 标记为 `[x]` 但移出列表）
   - 步骤 2 完成后：显示步骤 3-7
   - 如果剩余步骤少于 5 个，全部显示剩余步骤
   - 例如：只剩 3 个步骤时，显示全部 3 个
7. **技能使用表**：强烈建议列出每个步骤（或步骤组）需要使用的 skill，如果没有合适的 skill 可以为空
8. **禁止调用执行 agent**：主会话会自动调用规划中指定的 agent，planner 不要自己调用
9. **禁止自己执行**：planner 只负责规划，不负责实现任何代码
...
### 📝 执行步骤
- [ ] T1 - 初始化项目结构（创建目录、配置 TypeScript）
- [ ] T2 - 设计数据库模型（User 表、Session 表）
- [ ] T3 - 实现后端 API（并发）
- [ ] T4 - 实现前端页面（并发）
- [ ] T5 - JWT 认证集成
- [ ] T6 - 编写测试
