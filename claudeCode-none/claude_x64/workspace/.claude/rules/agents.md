...
>
> - **高复杂度**：强制使用 `brainstorming` → `planner` → 执行
> - **中低复杂度**：可选 `brainstorming` → `planner` → 执行
> - **简单任务**：直接执行
...
| "审查"、"检查质量" | **reviewer** |
| "漏洞"、"安全"、"审计" | **research** |
| "PoC"、"复现"、"漏洞验证" | **poc-exploit** skill + **research** |
| "测试"、"TDD" | **tdd-guide** |
...
- `brainstorming` - 参见 `CLAUDE.md` 中的复杂度判断规则
  - **高复杂度任务**：强制使用（多模块开发、系统重构、性能优化、安全审计、集成工作）
  - **中低复杂度任务**：用户明确要求时使用
