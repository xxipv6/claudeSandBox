...
## ⚠️ Analysis Mode 执行规则（强制，最高优先级）

**当进入 Analysis Mode 时，你必须立即执行以下步骤，不得跳过：**
...
**冲突处理优先级**：
1. **证据优先**：带证据的结论 > 无证据的结论
2. **安全优先**：安全 tester 的风险判断 > 其他判断
3. **边界优先**：QA 的边界判断 > 其他判断
4. **多数优先**：多数角色同意 > 少数意见
5. **保守优先**：有风险就标记，不假设安全
