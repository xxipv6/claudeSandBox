...
**重要**：使用 `tail -f` 实现真正的后台监控，持续读取日志！

**启动后台监控**：
```bash
# 使用 tail -f 持续监控日志文件（后台运行）
tail -f logs/app.log &
...
1. **使用 tail -f 后台监控**：启动 `tail -f` 持续监控日志文件，实时读取新增内容
2. **实时分析日志**：每当日志有新增内容，立即分析是否包含错误
3. **发现错误必须修复**：立即触发对应的 agent/skill，不等待用户确认
4. **修复后继续监控**：不要停止，tail -f 继续运行，继续分析日志
5. **仅限开发环境**：不用于生产环境

**真正的后台监控（强制执行）**：
```bash
# 启动监控（后台运行，实时推送日志）
tail -f logs/app.log          # 后端日志持续监控
tail -f npm-debug.log          # 前端日志持续监控
...
- ❌ 不要使用循环（while/for）
- ❌ 不要使用 sleep
- ❌ 不要使用 tail -n 50（只能读取最后 50 行，不是持续监控）
- ❌ 不要主动拉取日志（应该等待 tail -f 推送）
- ❌ 不要只说"监控中"而不实际启动监控
