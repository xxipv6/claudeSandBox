...
1. **强制规划流程**（最重要）：
   - **第一步**：收到任务后，立即停止，不要执行任何操作
   - **第二步**：调用 `planner` 智能体进行任务规划
   - **第三步**：planner 完成后，**自动调用规划中指定的智能体**执行
     - 如果规划指定"执行智能体：dev" → 自动调用 dev agent
     - 如果规划指定"执行智能体：research" → 自动调用 research agent
     - 如果规划指定"执行智能体：ops" → 自动调用 ops agent
   - **第四步**：**禁止自己执行代码**，必须由指定 agent 执行
   - **无需用户确认**：整个流程自动执行
